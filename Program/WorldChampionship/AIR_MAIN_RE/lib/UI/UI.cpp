    ...
    if(Serial4.read() == 255){
      ...
      switch(cmd){
        ...
        case 15:
          ...
          array_toSend[2] = 128;
          isToggle_ON = 0; // ★【削除】角度取得時に勝手にOFFにしないよう、この行をコメントアウトまたは削除します
          break;
        ...
        case 25:
          set_dir_request = 1;
          // if(isToggle_ON == 0) isToggle_ON = 1;
          // else isToggle_ON = 0;
          array_toSend[1] = 0;
          array_toSend[2] = 0;
          break;
        ...
        case 100:
          isToggle_ON = 0; // ESP32側から「ストップ(100)」が来たらしっかり止める
          break;
        
        case 150:
          isToggle_ON = 1; // ESP32側から「スタート(150)」が来たらしっかり動かす

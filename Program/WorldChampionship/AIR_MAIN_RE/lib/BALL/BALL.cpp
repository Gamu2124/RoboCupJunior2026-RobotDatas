...
void BALL::updateCatch()
{
  catch_val = analogRead(catch_photo_pin);
  
  if(catch_val < KICK_MIN || KICK_MAX < catch_val){
    isHold = 1;
  }
  else{
    isHold = 0;
  ...
  if(isHold_ave > 0.1 && abs(simplify.goPM(Ball_angle)) < 10){
    isBall_catch = 1;
  }
  else{
    isBall_catch = 0;
...
std::pair<float, int> BALL::around(int angle, float far, int type, int farval, int speed) //テスト
{
  ...
  // angle = simplify.FixLimit(angle-10);
  if(angle > 180)
  {
    originaly_ang = angle;
    angle = 360 - angle;
  ...
  if(type == 0) 
  {
    ...
    if(use_ang > 5){
      angle += use_ang * (far / 40); //近いとき4000の時 
    }
    else{
      angle = 0;
    }

    if(nomal_angle < 30 && angle != 0){
      speed *= 0.6;
    }
  }

...
void Defence::run(){
  ...
  if(Back_cam_angle_2 < 0){
    ...
    dash_judge_angle_2 = 320;
  }
  else{
    dash_judge_angle_1 = 40;
    dash_judge_angle_2 = 310;
  }
  ...
  if(A == 110) //ライントレース
  {
    ...
    if(B == A){
      ...
      else if(200 <= timer_hold.read_ms()){
        motor_speed = 250;
        if(700 <= timer_hold.read_ms()){
          active_Kick = 1;
        }
      }
      else if(dash_flag == 1){
